...
export async function registerUser(email: string, password: string, name: string) {
  ...
  return { ...tokenPair(user._id.toString(), user.email), user: { _id: user._id, email: user.email, name: user.name } }
}
...
export async function googleAuth(credential: string) {
  ...
  if (!user) {
    ...
    user = await User.create({ email, passwordHash, name: name || email.split('@')[0] })
  }
